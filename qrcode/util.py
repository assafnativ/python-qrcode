...
def optimal_data_chunks(data, minimum=4, error_correction=0):
    ...
    for version in xrange(1, MAX_VERSION + 1):
        data_list = encode_with_version(data, version, error_correction)
...
def generate_error_correction(buffer, rs_blocks):
    ...
    for rs_block in rs_blocks:
        ...
        result.append(reedsolo.RSCodec(rs_block.total_count - rs_block.data_count).encode(bytearray(chunk)))
    return result

def combain_data_and_error_correction(data_and_error_correction):
    result = []

    for chunk in izip_longest(*data_and_error_correction):
        for c in list(chunk):
            if None != c:
                result.append(c)
...
def add_edge(graph, source, char_index, char, mode_sizes):
    ...
    if char in ALPHA_NUM:
        bits_for_char = 6
        combined = 0
        if source.startswith('ALP_') and source.endswith('_0'):
            bits_for_mode = 0
            bits_for_char = 5
            combined = 1
        else:
...
def encode_with_version(data, version, error_correction):
    max_bits = BIT_LIMIT_TABLE[error_correction][version]
    bits = BitBuffer()
